...
for (i in c(1, 2, 3)) {
    ...
    label_df <- data.frame(x=rep(0.85, n),
                           y=rep(y, n),
...
# Add written words
intra_label_df <- data.frame(x=rep(0.775, n),
                             y=rep(32.5, n),
                             #label=append("vs.", rep("", n-1)),
                             label=rep("vs", n),
                             CHRGRP = rep("A", n),
                             NAME=mean_df$NAME)
intra_mean_label_df <- data.frame(x=rep(0.705, n),
                                  y=rep(32.5, n),
                                  label=roundMe(mean_df$DIV),
                                  CHRGRP = rep("A", n),
                                  NAME=mean_df$NAME)
...
p <- p + scale_x_continuous(n.breaks=11)

# No clipping
ggsave("div-by-species.pdf", height=7, width=7, dpi=300)
