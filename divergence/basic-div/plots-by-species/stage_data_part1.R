...
names <- c("hg002", "PanTro3", "PanPan1", "GorGor1", "PonAbe1", "PonPyg2",
           ...
           "hg002 vs. PonPyg2", "PonPyg2 vs. hg002",
           "PonAbe1 vs. PonPyg2", "PonPyg2 vs. PonAbe1",
           "PanTro3 vs. PanPan1", "PanPan1 vs. PanTro3")
...
#keep <- df %>%
#        group_by(NAME, CHRGRP, .add = TRUE) %>%
#        summarise_at(vars(GAP_OTHER, DIFF), funs(median(., na.rm=TRUE)))
#write.csv(keep,"median_summary.csv", row.names = FALSE)
...
#        group_by(NAME, CHRGRP, .add = TRUE) %>%
#        summarise_at(vars(GAP_OTHER, DIFF), funs(median(., na.rm=TRUE)))
#write.csv(keep,"median_summary-restrict-1MB.csv", row.names = FALSE)
